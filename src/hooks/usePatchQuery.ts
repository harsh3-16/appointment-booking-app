...
interface QueryResult<T> {
  ...
  data: T | null;
  error: any | null;
  patchQuery: ({ url, data, config }: { url: string; data?: any; config?: AxiosRequestConfig }) => Promise<void>;
}

export const usePatchQuery = <T>({ onSuccess, onFail }: { onSuccess?: (data: T) => void; onFail?: (err: any) => void } = {}): QueryResult<T> => {
  ...
  const patchQuery = useCallback(
    async (config: AxiosRequestConfig) => {
      ...
      try {
        const response = await apiClient.patch<ApiResponse<T>>(config.url!, config.data, config);
