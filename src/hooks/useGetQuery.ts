...
interface QueryResult<T> {
  ...
  data: T | null;
  error: any | null;
  getQuery: (config: AxiosRequestConfig) => Promise<void>;
}

export const useGetQuery = <T>({ onSuccess, onFail }: { onSuccess?: (data: T) => void; onFail?: (err: any) => void } = {}): QueryResult<T> => {
  ...
  const getQuery = useCallback(
    async (config: AxiosRequestConfig) => {
      ...
      try {
        const response = await apiClient.get<ApiResponse<T>>(config.url!, config);
