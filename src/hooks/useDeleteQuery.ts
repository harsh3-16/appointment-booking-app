...
interface QueryResult<T> {
  ...
  data: T | null;
  error: any | null;
  deleteQuery: ({ url, config }: { url: string; config?: AxiosRequestConfig }) => Promise<void>;
}

export const useDeleteQuery = <T>({ onSuccess, onFail }: { onSuccess?: (data: T) => void; onFail?: (err: any) => void } = {}): QueryResult<T> => {
  ...
  const deleteQuery = useCallback(
    async (config: AxiosRequestConfig) => {
      ...
      try {
        const response = await apiClient.delete<ApiResponse<T>>(config.url!, config);
